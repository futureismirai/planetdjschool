...
- **Booking**: レッスンID / 生徒名 / メールアドレス / 電話番号（任意） / 備考（任意） / リマインド送信済み日時
...
- **IndividualLesson**: 個別レッスン名 / 日時 / 講師名（管理者専用・生徒には非表示、1回1名まで）
- **IndividualParticipant**: 個別レッスンID / 生徒名 / メールアドレス（必須） / 備考 / リマインド送信済み日時
...
- `/admin`: カレンダー(月表示)。グループレッスン・体験会・個別レッスンの日程が表示され、クリックするとそれぞれの管理画面の該当箇所にジャンプする。前後の月への移動、月を直接指定しての移動が可能
- `/admin/lessons`（グループレッスン）: 全レッスン（過去分含む）の追加・編集・削除、予約状況・生徒連絡先・予約人数・残り枠数の確認を1画面で行える
- `/admin/trial`（体験会）: 体験会の参加者を管理（生徒には非公開。1回最大3名、生徒名・メールアドレス(必須)・備考を手入力で登録。登録時と開催3日前に確認メールを自動送信）
- `/admin/individual`（個別レッスン）: 個別レッスンの参加者を管理（生徒には非公開。1回1名まで、生徒名・メールアドレス(必須)・備考を手入力で登録。登録時と開催3日前に確認メールを自動送信）
...
## イベントオーガナイザーツール（`/organizer`）

DJイベントの現場運営を効率化するための管理者専用ツールです（`/admin`と同じログインで利用できます）。

### タイムテーブル作成（`/organizer/timetable`）

- イベントを作成し、開催日（複数日対応）・フロア（1日に複数フロア対応）を追加してタイムテーブルを管理する
- フロアごとに出演者名・開始終了時刻・出演者数を指定すると、タイムテーブルを自動作成
  - デフォルトは均等割り
  - 「ほぼ均等（5分単位）」「ほぼ均等（10分単位）」に区切り方を変更可能
  - 特定の出演者だけ時間を固定し、残りの出演者はその前後の空き時間で均等に配置することも可能
- 自動作成後もタイムテーブルは行ごとに自由に編集（時間・名前・SNSアカウント名の変更、並び替え、追加・削除）できる
- 各出演者の名前の隣にInstagramなどのSNSアカウント名を任意で入力可能
- 「SNSにコピー」ボタンでタイムテーブルをテキスト化してワンクリックでコピーできる

データモデル: `Event`（イベント） → `EventDay`（開催日） → `EventFloor`（フロア） → `TimetableSlot`（出演枠）

### 会場情報（`/organizer/venues`）

- 会場ごとに住所・アクセス・開催条件・機材情報・注意事項・宣材写真を保存してリスト化
- 各項目にコピー機能があり、他のスタッフへワンクリックで共有できる
- 宣材写真は複数枚アップロード可能（アップロード時にブラウザ側で自動リサイズ・圧縮）

データモデル: `Venue`（会場） → `VenuePhoto`（宣材写真）
