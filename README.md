...
- `/admin`: カレンダー(月表示)。レッスンと体験会の日程が表示され、クリックするとそれぞれの管理画面の該当箇所にジャンプする。前後の月への移動、月を直接指定しての移動が可能
- `/admin/lessons`: 全レッスン（過去分含む）の追加・編集・削除、予約状況・生徒連絡先・予約人数・残り枠数の確認を1画面で行える
- `/admin/trial`: 体験会の参加者を管理（生徒には非公開。1回最大3名、生徒名・メールアドレス(必須)・備考を手入力で登録。登録時と開催3日前に確認メールを自動送信）
